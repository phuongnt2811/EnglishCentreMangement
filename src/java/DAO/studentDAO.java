...
public class studentDAO {
    ...
    public List<Student> getAllStudents() {
        ...
        String sql = "SELECT * FROM StudentHE151412 inner join [AccountHE151412] on StudentHE151412.loginId = [AccountHE151412].id";
        try {
...
//            ps.setInt(8, student.getLoginId());
    ...
    public void update(Student student) {
        String sql = "UPDATE StudentHE151412\n"
                + "   SET [Fullname] = ?\n"
                + "      ,[code] = ?\n"
                + "      ,[Gender] = ?\n"
                + "      ,[Phone] = ?\n"
...
//        System.out.println(count);
    ...
    public List<Student> getpage(int index) {
        List<Student> list = new ArrayList<>();
        String sql = "select * from ( select  ROW_NUMBER() OVER(ORDER BY id) as [rownumber] ,* from StudentHE151412 )as TBL where [rownumber] between ? and ?  order by ID";
        try {
            Connection conn = new DBContext().getConnection();
            PreparedStatement ps = conn.prepareStatement(sql);
